"""0화 기획 초안이 RUN_CONTRACT.json을 지켰는지 검사한다.

사용법:
    python check_draft.py 초안.md
    python check_draft.py 초안.md 다른잠금.json
...
def check(draft_text, contract):
    fails = []
    draft_norm = norm(draft_text)

    for name, sentence in contract["잠금_문장"].items():
        if norm(sentence) not in draft_norm:
            fails.append(f"잠금 {name} 원문이 없거나 바뀜: {sentence}")

    for sentence in contract["반드시_들어갈_문장"]:
        if norm(sentence) not in draft_norm:
            fails.append(f"필수 문장 없음: {sentence}")

    groups = [
        ("없는 자료를 있다고 씀", contract["없는_자료를_있다고_쓰는_말"]),
        ("있는 자료를 없다고 씀", contract["있는_자료를_없다고_쓰는_말"]),
    ]
...
def main():
    if len(sys.argv) not in (2, 3):
        print(__doc__)
        return 2
    contract_path = sys.argv[2] if len(sys.argv) == 3 else HERE / "RUN_CONTRACT.json"
    contract = json.loads(load(contract_path))
    fails = check(load(sys.argv[1]), contract)
    if not fails:
        print("PASS")
        return 0
