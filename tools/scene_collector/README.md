...
**장면 수를 미리 자르지 않는다.** 활성 작품 안에 실제로 있는 정확 동일표현 장면을 가능한 한 다 찾는 것이 목적이라, 공식 커서로 다음 후보가 없을 때까지 이어서 훑는다. 일반 검색과 정확 검색을 각각 끝까지 훑어 결과를 합치고 장면 ID로 중복을 제거한다. 페이지 순회가 비정상으로 끝나면(같은 자리 반복, 더 있다면서 다음 위치 없음) 모은 것만 조용히 완료로 보여주지 않고 명확한 오류를 낸다.
