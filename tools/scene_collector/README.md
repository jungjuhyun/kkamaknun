...
찾은 장면 수를 제한하는 설정은 없다. 고른 표현 하나를 여러 장면에서 반복해 보여줄 재료를 모으는 것이 목적이라, 검색이 매칭한 결과는 페이지 끝까지 모두 받아 온다. **"장면 수 제한 없음"과 "작품 안의 모든 출현을 찾음"은 다른 계약이며, 뒤의 것은 주장하지 않는다** — 아래 [검색 회수 계약](#검색-회수-계약)을 보라.

옛 `search.candidate_count`·`search.nadeshiko_take`·`search.scene_result_limit` 키는 더 이상 사용하지 않는다. 남아 있어도 오류가 나지는 않지만 값은 승계되지 않는다. `expression_generation_limit`은 설정 탭에서 직접 바꿔 저장할 수 있다.
...
검색은 활성 선호작만 대상으로 하며(활성 작품이 없으면 `NoActiveMediaError`), **장면 수를 미리 자르지 않는다.** 공식 SDK 일반 검색과 `exact_match=True` 검색을 각각 공식 커서로 다음 후보가 없을 때까지 이어서 훑고, 받은 장면을 ID 기준으로 합친 뒤 기존 표면형 판정을 적용한다. 커서 순회는 화수·위치 기준 `TIME_ASC` 정렬로 고정한다 — 공식 검색은 Elasticsearch `search_after`(keyset) 방식이라 기본 `RELEVANCE` 정렬은 점수 동점에서 순회 자리가 흔들릴 수 있고, 커서는 요청의 정렬 구성과 짝이 맞아야 하므로 페이지 도중 정렬을 바꾸지 않는다. 페이지 순회가 비정상으로 끝나면(같은 자리 반복, 더 있다면서 다음 위치 없음) 모은 것만 조용히 완료로 보여주지 않고 명확한 오류를 낸다. **검색 응답과 결과는 DB에 저장하거나 캐시하지 않는다.** 같은 표현을 나중에 다시 찾으면 Nadeshiko를 다시 호출한다.

장면이 수십 개 나오는 것은 정상이다. 목록은 텍스트로만 보여주고 영상은 고른 장면 하나만 불러온다.
...
세 가지를 서로 다른 계약으로 구분한다. 코드·화면·문서 어디에서도 섞어 쓰지 않는다.

1. **장면 수 제한 없음** — 참이다. 3개/5개/20개 같은 상한도, 정상 페이지 순회의 페이지 상한도 없다.
2. **검색이 매칭한 결과를 빠짐없이 확인** — 이제 검증한다. 일반 검색과 `exact_match=True` 검색은 **경로마다 따로** 판정한다. 경로별로 같은 조건의 공식 `get_search_stats`(경로당 요청 1회)로 작품별 매칭 수를 받아 두고, 그 경로가 페이지를 끝까지 넘겨 실제로 받은 서로 다른 장면 수와 작품마다 대조한다(`SourceCoverage.normal` / `SourceCoverage.exact`). 한 경로라도 모자라면 검증 실패다. 모자란 작품이 있으면 조용히 완전한 척하지 않고 화면에 알린다.

   **경로를 합친 뒤에 세면 안 된다.** 정확 검색이 일반 검색에 없던 장면을 더 주는 경우가 실제로 있어서, 합계로 비교하면 그 추가분이 일반 경로의 누락을 메워 검증이 통과해 버린다(일반 통계 10건, 일반 회수 9건, 정확에만 있는 장면 1건 → 합계 10). 공식 검색 통계가 `exact_match` 조건을 실제로 반영한다는 것은 실측으로 확인했다: `大丈夫です`는 일반 351건 / 정확 47건, `ありがとう`는 177건 / 151건으로 갈린다. 경로별 회수 수는 그 경로 안에서 장면 ID로 중복을 제거해 세므로, 같은 장면을 두 번 받아도 누락을 가리지 못한다.
3. **작품 안의 모든 문자열 출현을 찾음** — **주장하지 않는다.** 공식 검색은 Elasticsearch 형태소 분석 기반이라 문자열 일치가 아니다. 실측 예: 귀멸의 칼날 전 대사 5,597건을 열거해 세면 `ごめん`이 문자열로 29건 들어 있지만 검색이 매칭해 주는 것은 19건이다(`ごめんなさい`는 다른 토큰이라 잡히지 않는다). 그 10건은 표면형 판정도 탈락시키므로 최종 결과는 같았지만, "검색이 잡지 않은 출현"이 원리상 존재할 수 있다는 사실은 그대로다.

`SourceCoverage`는 세션 동안만 살고 DB에 저장하지 않으며, 사용자 화면에는 작품 공개 ID를 노출하지 않는다.

전체 열거가 필요하면 공식 queryless browse(`query` 생략 + 작품/화 필터 + `TIME_ASC`)로 작품의 전 대사를 순회할 수 있다. 귀멸의 칼날에서 5,597건을 받아 `get_media`의 `segment_count`와 정확히 일치함을 확인했다. 다만 작품 하나에 113요청이 들고 계정 월 쿼터가 5,000요청이라 **제품 검색 경로에는 쓰지 않는다.** 진단·검증 용도로만 둔다.
...
**이번 범위에서 로컬 자막 결과는 그 표현이 자막 작품에 존재하는지 확인하는 위치 후보다.** 영상 재생·판정 저장·`work_scenes` 저장·내보내기는 Nadeshiko 장면만 지원한다. 로컬 결과는 작품 표시명·화수·start/end ms를 담고 있어 사용자가 보유한 원본 영상에서 위치를 찾는 데 쓰며, 자막과 원본의 판본 차이로 실측 기준 ±15초 안팎의 오프셋이 있을 수 있고 화마다 다를 수 있다. 검색 결과 화면은 **제작 가능 장면(Nadeshiko)과 로컬 자막 위치 후보를 절대 합산하지 않고** 각각의 수와 합계를 구분해 보여준다.

### 프랜차이즈 source unit과 자막 색인 (UAT FIX 3-A)

사용자 선택 단위(프랜차이즈)와 실제 검색 자료 단위(source unit)를 분리한다. curated 항목에 `source_units`가 있으면(현재 귀멸의 칼날) 선호 작품 탭에서 항목 아래에 unit별 준비 상태가 보인다: `장면 검색·제작 가능`(Nadeshiko) / `로컬 자막 색인됨 · 위치 확인만 가능` / `자막 준비 필요` / `현재 검색 불가`(예: 무한성편 극장판 — 텍스트 자막 없음). **프랜차이즈 체크 한 번**이 Nadeshiko 항목과 색인된 unit 로컬 자막을 함께 활성/비활성하며, unit을 시즌별로 따로 체크하지 않는다.

unit의 [자막 색인] 버튼으로 자막 폴더를 지정하면 프로그램이 그 unit의 canonical 표시명(`귀멸의 칼날 · 유곽편` 형식)으로 색인·연결한다 — **문자열 접두어 추측 연결은 쓰지 않는다.** 색인 전 검증 계약: source unit 폴더에는 사용할 자막 판본 하나만 둔다. TV는 화당 파일 1개(화수 인식 불가 파일·같은 화 판본 중복은 색인 거절), 극장판은 파일 정확히 1개. 색인 결과로 자막 파일 수·인식 화수·대사 수를 보여준다. 체크가 꺼진 상태에서 색인하면 색인만 되고 검색 대상에는 들어가지 않는다(체크를 켤 때 함께 활성).
...
1. **설정** 탭에서 작업 데이터 위치·AI 서비스/모델·Nadeshiko 키 상태를 확인한다. 키 값 자체는 화면에 표시하지 않는다. 표현 생성 상한은 여기서 바꿔 `설정 저장`을 누르면 설정 파일에 저장되고 지금 실행에도 바로 반영된다. 찾을 장면 수를 정하는 설정은 없다. 작업 데이터 위치는 실행 중에 바꾸면 열려 있는 데이터베이스와 어긋나므로 파일에서만 바꾼다.
2. **선호 작품** 탭에서 작품을 활성화한다. 상단의 **추천 후보 목록(curated 97개, A군 63/B군 34)**에서 체크하면 그 항목에 연결된 Nadeshiko entry들이 함께 저장·활성화되고(예: 체인소 맨 = TV + 레제편), 해제하면 함께 비활성화된다. `source_units`가 있는 항목(현재 귀멸의 칼날)은 항목 아래에 unit별 준비 상태와 [자막 색인] 버튼이 보이고, 체크 한 번이 Nadeshiko와 색인된 unit 로컬 자막을 함께 켜고 끈다. 전체/A군/B군 필터가 있고, 각 항목에 자료 상태(`장면 검색 바로 가능`/`연결된 일부만 장면 검색 가능 · 연결 N개`/unit 요약 `검색 가능 n개 · 자막 준비 필요 m개`/`일본어 자막 준비 필요`)가 표시된다. 부분 지원 작품은 시리즈 전체가 아니라 현재 연결된 항목만 검색된다는 뜻이다. 선별에 쓴 내부 등급은 화면에 보여주지 않는다. unit이 없는 `일본어 자막 준비 필요` 항목은 자막을 확보해 `index_local_subtitles`로 **curated 한국 제목과 같은 표시명**으로 색인한 뒤에만 체크할 수 있다(자동 다운로드 없음). curated 목록에 없는 작품은 아래의 작품 직접 검색으로 기존처럼 추가한다. 활성 작품이 하나도 없으면 검색은 실행되지 않고 "검색에 사용할 활성 작품이 없습니다" 안내가 표시된다. 97개 후보가 DB에 자동 삽입되지는 않으며, 체크한 작품만 저장된다.
...
5. **장면 검수** 탭은 현재 작업 맥락(한국어 의미 → 일본어 표현)을 보여주고, 찾은 정확 동일표현 장면을 전부 텍스트 목록으로 표시한다. 장면이 수십 개여도 영상은 고른 하나만 불러온다. `이 장면 보기`를 누르면 **영상 플레이어 하나에 그 장면만** 로딩된다. 필요할 때만 `문맥 조회 + 한국어 번역`을 실행하고, 채택/예비/제외와 메모를 저장한다. 로컬 자막 결과는 그 표현이 자막 작품에 있는지 확인하는 참고 표시이며 영상·판정 대상이 아니다. 새 의미를 조회하거나 다른 표현을 고르면 이전 장면 목록·영상·번역 표시는 모두 지워지고, 영상은 장면을 다시 고른 뒤에만 나타난다.
