...
2. **검색이 매칭한 결과를 빠짐없이 확인** — 이제 검증한다. 검색 전에 공식 `get_search_stats`(요청 1회)로 작품별 매칭 수를 받아 두고, 페이지를 끝까지 넘겨 실제로 받은 수와 작품마다 대조한다(`SelectedExpressionScenes.coverage`). 모자란 작품이 있으면 조용히 완전한 척하지 않고 화면에 알린다.
