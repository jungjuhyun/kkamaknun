...
찾은 장면 수를 제한하는 설정은 없다. 고른 표현 하나를 여러 장면에서 반복해 보여줄 재료를 모으는 것이 목적이라, 활성 작품 안에 있는 정확 동일표현 장면을 가능한 한 다 찾는다.
...
검색은 활성 선호작만 대상으로 하며(활성 작품이 없으면 `NoActiveMediaError`), **장면 수를 미리 자르지 않는다.** 활성 작품 안에 실제로 있는 정확 동일표현 장면을 가능한 한 다 찾는 것이 목적이라, 공식 SDK 일반 검색과 `exact_match=True` 검색을 각각 공식 커서로 다음 후보가 없을 때까지 이어서 훑고, 두 경로에서 기존 표면형 판정을 통과한 결과를 장면 ID 기준으로 합친다. 페이지 순회가 비정상으로 끝나면(같은 자리 반복, 더 있다면서 다음 위치 없음) 모은 것만 조용히 완료로 보여주지 않고 명확한 오류를 낸다. **검색 응답과 결과는 DB에 저장하거나 캐시하지 않는다.** 같은 표현을 나중에 다시 찾으면 Nadeshiko를 다시 호출한다.
