...
Normal `--resume-archive` always requires every archived baseline prompt to equal the current prompt. The only exception is the explicit `--resume-surrogate-impact` continuation mode, which is restricted to the repository-owned `core_final_full_baseline_resume_94_surrogate_contract_impact.json` and its exact SHA-256-named recovered archive. It validates that artifact's snapshot, lane, task identities, raw status counts, fixture/scorer/schema/task manifest, prompt artifacts and affected-record scope, then imports only the unaffected valid records as a release seed. It does not promote `historical_audit` to a runnable path, cannot accept another archive or verdict, and records the excluded records and current prompt contract in the new evidence. At the current checkpoint this produces a 64-valid seed and exactly 48 remaining valid trials.
