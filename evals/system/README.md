...
Source manifests are now LF/CRLF-independent, and inherited archive names are parsed independently of host path syntax. Legacy archives remain hard-rejected when any source hash differs beyond line endings. The recovered 94-trial archive imports on the original Windows measurement checkout; the current Linux checkout exposes a remaining legacy `schema.py` manifest mismatch, so cross-platform release-gate reuse is not yet frozen even though its raw counts and A02 envelopes are preserved and auditable.
