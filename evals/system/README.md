...
`P4-A` accepts a bootstrap PASS only when the raw user-visible receipt is part of the captured final response, both byte strings match stored SHA-256 receipts, and the receipt line matches the actual `bootstrap: OK — <ref>@<short_sha> / <owners>` grammar. Runtime fixture data keeps selector identity, expected receipt ref, and immutable snapshot in separate fields: a selector such as a PR identity is not the work ref printed by the receipt. The parsed ref and seven-character SHA must exactly match the expected receipt ref and snapshot, and the owner set must include `STATE.md`, `AGENTS.md`, and routed `evals/system/README.md`; order is irrelevant. Synthetic `BOOTSTRAP_SUCCESS`/`bootstrap ready` aliases are not success receipts. A missing receipt is `UNKNOWN`; a present malformed or wrong receipt is `FAIL`; broken raw/structured linkage is `INVALID_FIXTURE`. The natural target prompt contains no receipt or output-format oracle.
