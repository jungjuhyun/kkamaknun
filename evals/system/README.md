...
- Actual ChatGPT Work target: Phase 4 observable-evidence UAT executed; corrected offline classification is `P4-A UNKNOWN / P4-B PASS / P4-C1 PASS / P4-C2 PASS / P4-D PASS`, aggregate `PHASE4_PILOT_BLOCKED` (`4/5 PASS`). The earlier classifier-defect aggregate `PHASE4_PILOT_FAILED` remains historical evidence, not confirmed product failure.
...
Phase 4 uses two explicit clocks. **Repository-snapshot truth** is the immutable STATE/owner truth authenticated for a trial; it is authoritative when the target is asked for repository-current state. The initial preserved pilot fixture therefore legitimately says Phase 3 `PHASE3_PASSED`, Phase 4 `PHASE4_PILOT_NOT_RUN`, and Actual Work call count `0`, even though external execution occurs later. **External pilot evidence** records the later measurement lifecycle—capability receipts, calls, responses, markers, observations, classifications, and aggregate—and never rewrites repository-snapshot truth retroactively. A target answer is not a semantic failure merely because its authenticated snapshot still says NOT_RUN/0 calls during later measurement; conversely, promoting a live external aggregate to that snapshot's repository truth is a contradiction. Contract/classifier correction completion is also distinct from Phase 4 pilot completion.

The pilot asks only whether Phase 3's core observable contracts remain true on the actual Work surface. It may grade the raw user input and final response, cryptographically linked receipt capture, product-visible activity, independently captured Work/project setup, post-run marker reveal, and external GitHub/repository before-and-after state. `WorkObservableEvidence` records availability and a diagnostic note only: it has no `correct` field and no stored human boolean can authorize PASS. The classifier recomputes each scenario's bounded assertions from captured material after presentation-only Markdown normalization. Positive tokens alone do not establish an owner, status, provenance, separation, or precedence relation. Owner claims remain subject-linked, so `P4-B` current-state/routing ownership and `P4-D` System Evaluation measurement ownership do not contaminate each other. P4-B recognizes bounded English/Korean current-state claims, the exact canonical shorthand, a delegated `evals/system/README.md` detail-source relation, non-replacement of `STATE.md` ownership, and historical-only/current-precedence relations across adjacent sentences. Explicit same-domain contradictions outrank correct boilerplate. Free-form meaning outside the bounded grammar is `UNKNOWN`; observation infrastructure failure is `INFRA_ERROR`; invalid setup, provenance, linkage, or chronology is `INVALID_FIXTURE`. The contract never infers complete reads, retrieval, tool use, personal-context use, reasoning, or fallback paths from correct wording.
...
The initial pilot denominator is exactly the five runnable scenarios `P4-A/B/C1/C2/D`, once each. All five executed PASS yields `PHASE4_PILOT_PASSED`; a corrected deterministic semantic product FAIL yields `PHASE4_PILOT_FAILED`; otherwise an incomplete, `UNKNOWN`, `INFRA_ERROR`, or `INVALID_FIXTURE` runnable set yields `PHASE4_PILOT_BLOCKED`; no execution yields `PHASE4_PILOT_NOT_RUN`. Capability-blocked and safe-fixture-deferred scenarios are reported separately and never inflate the PASS numerator. Historical classifier-produced FAIL values remain evidence history and are not product-failure proof after the classifier defect is established; the aggregate must be recomputed from offline classifications rather than forced. P4-D distinguishes Phase 3 PASS from Actual Work acceptance, treats Phase 4 as separate observable Actual Work UAT, accepts contract/classifier correction completion without calling the pilot complete, requires Phase 5 `NOT_STARTED`, and rejects video-planning substitution. Pilot PASS would mean only that this initial observable UAT feasibility pilot passed, not Phase 4 completion, Work reliability certification, or a release gate. Phase 3 remains `PHASE3_PASSED`; Phase 5 remains `NOT_STARTED`.
