...
!! 표본 한계 — 이 파일로 잰 숫자로 작품 순위를 말하지 마라 (2026-08-20) !!
아래 dict는 **ImmersionKit 색인 안에 있는 것만**이다. ImmersionKit 80편 중
라프텔 인기 100위 프랜차이즈는 9개뿐이라(실측/재료교차표.txt),
소년만화 dict에는 다음이 통째로 없다:
  은혼(1위) 주술회전(2위) 전생슬라임(7위) 하이큐(10위) 진격의거인(11위)
  스파이패밀리(13위) 체인소맨(25위) 원피스(66위) 블리치(76위) 나루토 코난 짱구
극장판 dict에도 스즈메·슬램덩크가 없다.

→ 표현모으기.py 결과는 **항목별 재고의 하한치**로만 쓴다.
   작품 간 비교가 필요하면 kitsunekko 자막 전수 카운트로 따로 잰다
   (방법: 실측/JLPT_N5문법_재고.txt [6]번).
   실제로 이 한계를 안 보고 작품 순위 결론을 냈다가 철회한 적이 있다 — CLAUDE.md 참조.
"""
