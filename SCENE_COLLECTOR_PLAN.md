...
상태: **개발 진행 중 / 작업 0·1·2·3·4 완료 / 다음 작업 5**
...
현재 병목은 영어 fallback 부족이 아니라 Nadeshiko 일반 검색의 부분 일치 거짓 양성이므로 영어 fallback은 아직 구현하지 않는다.
...
따라서 작업 5에서는 단순히 검색 결과가 존재한다는 이유로 채택하지 않고, **원문 안에서 목표 표면형이 실제 표현으로 성립하는지 로컬에서 검사**한다.
...
작업 0~3 기술 확인은 완료됐으며 남은 개발량은 다음 계획 검토에서 다시 산정한다.
...
**Codex 작업 5 — 정확 동일표현 검사**부터 진행한다.

작업 0 — 개발 골격, 작업 1 — 설정 로딩·검증, 작업 2 — Nadeshiko 실제 연결 확인, 작업 3 — AI 실제 연결 확인, 작업 4 — 한국어 표현 찾기는 완료되어 `main`에 반영됐다.
작업 4 실제 평가에서는 10/10 입력에서 일본어 후보 경로만으로 corpus-backed 결과가 나왔고 영어 fallback 필요성은 확인되지 않았다. 대신 Nadeshiko 일반 검색의 부분 일치 때문에 `悪い → 気持ち悪い`, `ほんとそれ → 분리된 ほんと/それ`, `ん？なんて？ → 인용의 なんて`, `今、何してるんですか？ → 비슷한 다른 문장` 같은 거짓 양성이 관찰됐다.
작업 5에서는 Unicode·공백·문장부호 같은 표기 차이만 필요한 범위에서 정규화하고, 문법 형태를 임의로 바꾸지 않은 채 실제 원문에 목표 표면형이 같은 표현으로 존재하는 장면만 남긴다. 한자/가나 허용 표기는 문법 변형과 분리해 관리한다.
SQLite/캐시, 선호작 관리, 번역, UI, 영상 저장은 같은 변경에 섞지 않는다.