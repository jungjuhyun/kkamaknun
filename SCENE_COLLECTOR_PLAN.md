...
상태: **개발 진행 중 / 작업 0·1·2 완료 / 다음 작업 3**
...
**Codex 작업 3 — AI 실제 연결 확인**부터 진행한다.

작업 0 — 개발 골격, 작업 1 — 설정 로딩·검증, 작업 2 — Nadeshiko 실제 연결 확인은 완료되어 `main`에 반영됐다.
작업 3에서는 Instructor를 사용해 서로 다른 AI 서비스 최소 2개를 설정만 바꿔 실제 연결하고, 애플리케이션 검색 로직을 수정하지 않은 채 동일한 Pydantic 자료형이 반환되는지 검증한다.
일반 자동시험에서는 실제 AI API 비용·사용량을 발생시키지 않으며, 작업 4의 한국어 표현 검색 품질 개발이나 이후 저장·화면 기능을 같은 변경에 섞지 않는다.
작업 0~3 기술 확인을 마친 뒤 한국어 → 일본어 후보 → 실제 애니 대사 회수 품질 개발로 넘어간다.