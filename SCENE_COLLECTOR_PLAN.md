...
상태: **개발 진행 중 / 작업 0·1·2·3·4·5 완료 / 작업 6 진입 전 검색 recall 검증**
...
현재 병목은 영어 fallback 부족이 아니라 정확 동일표현의 회수 범위이므로 영어 fallback은 아직 구현하지 않는다.
...
작업 5 live 검증에서는 일부 target이 상위 20건에서 정확 표면형 0건이었다. 정확 표현이 더 뒤 페이지에 있을 가능성을 상위 N건만으로 `없음` 처리하지 않도록, 작업 6 진입 전에 공식 `iter_search` 또는 cursor pagination을 이용해 **정확 surface를 필요한 개수만 확보할 때까지만 추가 페이지를 조회하는 최소 recall 검증**을 수행한다. 무제한 순회나 새 검색엔진은 만들지 않는다.
...
작업 번호를 새로 만들지 않는 단기 검증이다.
작업 5 live에서 `ほんとそれ`, `ん？なんて？`, `今、何してるんですか？`가 상위 20건에서 정확 surface 0건이었던 관찰을 확인한다.

목적:
- 상위 N건 0개를 corpus 전체 0개로 잘못 판정하지 않는지 확인
- Nadeshiko 공식 `iter_search` 또는 cursor pagination 재사용
- 정확 surface를 필요한 개수만 찾으면 즉시 중단
- 합리적인 최대 조회량을 두어 무제한 순회 방지
- 새 검색엔진·새 tokenizer·영어 fallback을 이 검증 때문에 추가하지 않음

이 검증으로 기존 pagination만으로 충분한 회수 범위를 확보할 수 있으면 바로 작업 6으로 이동한다.
...
작업 0~5의 핵심 검색·연결 기술 검증은 완료됐으며, 검색 recall 검증 뒤 남은 개발량을 기존 작업 6~13 기준으로 이어간다.
...
**작업 6 진입 전 검색 recall 검증**부터 진행한다.

작업 0 — 개발 골격, 작업 1 — 설정 로딩·검증, 작업 2 — Nadeshiko 실제 연결 확인, 작업 3 — AI 실제 연결 확인, 작업 4 — 한국어 표현 찾기, 작업 5 — 정확 동일표현 검사는 완료되어 `main`에 반영됐다.
작업 5는 일반 검색 결과에 로컬 surface matcher를 적용하고, 정확 결과가 없을 때만 Nadeshiko `exact_match=True`를 fallback으로 사용하도록 확정했다. 실제 거짓 양성은 제거했지만 `ほんとそれ`, `ん？なんて？`, `今、何してるんですか？`가 상위 20건에서 정확 surface 0건이어서 recall 위험이 새로 관찰됐다.
다음 한 단계에서는 Nadeshiko가 이미 제공하는 `iter_search` 또는 cursor pagination을 재사용해, 정확 surface를 필요한 개수만 찾을 때까지 제한적으로 추가 페이지를 확인한다. 합리적인 최대 조회량 안에서도 회수가 안 되는지 확인해 상위 N건 0개와 corpus 전체 0개를 구분한다.
이 검증으로 기존 SDK pagination만으로 충분하면 새로운 검색 계층을 만들지 않고 기존 계획의 작업 6 — SQLite·캐시·자료구조 버전으로 바로 넘어간다.
영어 fallback, 새 tokenizer, 선호작 관리, 번역, UI, 영상 저장은 이 recall 검증에 섞지 않는다.