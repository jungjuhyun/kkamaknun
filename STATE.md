...
이 값은 확정 계약 시간이 아니라 불확실성 관리용이다. 작업 0~10의 검색·저장·캐시·선호작·번역·화면 기술·사용자 화면 검증과 로컬 자막 fallback 확장까지 모두 main에 반영됐다. 개발의 다음 작업은 기존 계획의 작업 11이다.
...
16. 작업 11 — 영상 저장·내보내기: **다음 작업**. 채택 MP4 저장, 제작용 JSON/CSV 출력, 중복 다운로드 방지.
