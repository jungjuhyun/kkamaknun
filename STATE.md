...
`claude/scene-collector-uat-fixes-1` 브랜치에서 수정 중이다. 3번 때문에 검색 회수 설계가 바뀌어, 공식 SDK의 커서 페이지 순회를 목표 수를 채울 때까지만 쓰고 호출량에 안전 상한을 두는 방식으로 정리했다. 설정 `nadeshiko_take`는 뜻이 달라져 `scene_result_limit`으로 바꾸고 값을 승계하지 않는다.

재-UAT에서 **장면 수집기가 고정 개수 표시 중심으로 구현되어 제작 재료 최대 수집 목적과 충돌**하는 문제가 추가로 확인됐다. `大丈夫です` 검색이 3건만 나온 것은 실제 장면이 3개라서가 아니라 프로그램이 충분히 찾기 전에 멈추는 구조였기 때문이다. 이 도구는 표현 하나를 여러 장면에서 반복해 보여줄 재료를 모으는 것이 목적이므로, 장면 수를 자르는 설정(`scene_result_limit`)과 페이지 상한을 없애고 활성 작품 범위에서 pagination을 끝까지 확인하도록 고쳤다. 일반 검색과 exact_match 검색 결과를 합치고 장면 ID로 중복을 제거한다.
