...
이번 POC의 계약·판정 기록은 `plans/EP0_PLANNING_RELIABILITY_POC.md`, 단계 정의는 `tools/harness/PIPELINE.yaml`, 진행 상태는 `tools/harness/STATE.json`이 owner다.
...
- 기존 `15~20분 실제 진단용 촬영`, `1화 34개 후보 선별`, `장면 수집기 UAT 재작업`은 모두 보존한다. **재관찰이 끝나기 전에는 이 셋을 현재 작업으로 취급하지 않는다.** 사용자가 다른 작업을 선택하면 그 요청을 우선하며, 그래도 이 다음 할 일은 폐기되지 않는다.
...
현재 해석표의 owner는 `materials/ep0_take1/chainsawman_take_tables_v2.xlsx`이고, 판정 기준의 owner는 `materials/ep0_take1/해석표_판정기준.md`다.
`ep0_take1_tables_v2.xlsx`와 `해석표_v2.md`는 이전 단계 기록이며 현재 기준이 아니다.
