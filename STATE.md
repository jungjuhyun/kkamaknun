...
이 값은 확정 계약 시간이 아니라 불확실성 관리용이다. 작업 0~5의 핵심 검색·연결 기술 검증과 검색 recall 검증은 완료됐으며, 이제 기존 계획의 작업 6으로 넘어간다.

## 지금 딱 한 단계

**0화 구성은 계속 유지하면서, 장면 수집기의 작업 6 — SQLite·캐시·자료구조 버전을 진행한다.**
...
10. 작업 6 — SQLite·캐시·자료구조 버전: **다음 작업**.
11. 실제 검색 품질과 검수 병목이 확인된 뒤에만 화면과 추가 자동화를 붙인다.

즉 현재 우선순위는 새로운 검색 시스템을 더 만드는 것이 아니라 **0화 구성 + 검증된 검색 흐름을 저장·재사용 가능한 로컬 작업 상태로 연결하는 것**이다.
