...
현재 기획 흐름에서 다음으로 이어가기 좋은 단계는 **채택한 프로토콜로 약 15~20분의 실제 진단용 촬영본을 먼저 확보하는 것**이다.
- 모든 문장마다 멈추지 않고, 청취 성공·부분 청취·의미만 이해·중요 미청취·강한 확인 욕구가 스스로 의식되는 순간만 멈춘다.
- 촬영 중에는 정답을 미리 확인하지 않고 당시 청취·이해·반응을 가능한 한 원형 그대로 남긴다.
- 15~20분 촬영본에서 멈춤 빈도, 발언 길이, 소리/뜻 분리 기록, 시청 몰입, 재미·진단 재료가 충분한지 확인한다.
- 프로토콜이 작동하면 같은 방식으로 본 촬영을 이어간다.
- 실제 대사 정답 확인과 `정확/부분/추론/오답/미청취` 판정은 촬영 후 별도 단계에서 붙인다.
- 본 촬영이 확보된 뒤에야 3층 C가 실제로 드러나는 콘텐츠 각과 핵심 장면을 선별한다.
...
즉 현재 기획상 권장 체크포인트는 **채택한 0화 진단용 촬영 프로토콜로 15~20분 실제 촬영본 확보**이고, 장면 수집기는 **미완료 보존 작업**이다. 사용자가 다른 작업을 선택하면 그 요청을 우선한다.
