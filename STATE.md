...
이 값은 확정 계약 시간이 아니라 불확실성 관리용이다. 작업 0~10의 검색·저장·캐시·선호작·번역·화면 기술·사용자 화면 검증과 로컬 자막 fallback 확장까지 모두 main에 반영됐다. 개발의 다음 작업은 **작업 10.5 — curated 작품 풀 + 체크 활성화 UI**로 변경됐고, 작업 11은 그 완료·검수·merge 후 진행한다.

## Curated 후보 작품 풀 — 조사 완료

한국 인기/인지도 기준 curated 애니 후보 풀 조사를 완료했고 사용자가 채택했다.

- 현재 후보 **97개: A군 63 / B군 34** (Tier 1/2/3, 한국 인기 근거 A/B/C 등급 유지).
- **Nadeshiko 직접 경로 59개**, 나머지 **38개는 Jimaku에서 실제 비압축 일본어 SRT/ASS 파일 경로를 확인**했다.
- 따라서 97/97은 **"실사용 검증 완료"가 아니라 "사용 가능한 소스 경로 확인 완료"**다. Jimaku 경로 작품은 실제 색인 시 판본·타임코드·품질 확인이 필요하다.
- 최종 활성 작품 선택은 채팅 표에서 미리 고르는 방식이 아니라 **사용자가 실제 UI에서 체크**하는 방식으로 한다. AI가 자동으로 활성화하지 않는다.

## 지금 딱 한 단계

**0화 구성은 계속 유지하면서, 장면 수집기의 작업 10.5 — curated 작품 풀 + 체크 활성화 UI를 진행한다.**
...
16. 작업 10.5 — curated 작품 풀 + 체크 활성화 UI: **다음 작업**. 후보 97개를 UI에 표시하고 사용자 체크만으로 기존 `is_active` 검색 pool을 바꾼다.
17. 작업 11 — 영상 저장·내보내기: **작업 10.5 완료·검수·merge 후 진행**.

즉 현재 우선순위는 새로운 검색·번역 알고리즘을 더 만드는 것이 아니라 **0화 구성 + curated 후보를 사용자가 체크해 검색 대상을 실제로 넓히는 작업 10.5**다.
