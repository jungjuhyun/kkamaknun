...
- 다음 exact action은 명시적 실행 승인을 받은 뒤 same snapshot/lane에서 `--resume-archive evals/system/evidence/phase3_resume_94_of_112_recovered.zip`을 사용해 remaining 18 valid trials만 이어서 실행하는 것이다. 완료 trial을 재실행하거나 다른 snapshot/lane과 혼합하지 않으며 Phase 4는 계속 시작하지 않는다.
