...
현재 기획 흐름에서 다음으로 이어가기 좋은 단계는 **이 3층 C가 실제로 드러나는 콘텐츠 각과 핵심 장면을 확정하는 것**이다.
- 먼저 확보된 실제 촬영 재료에서 `이 영상이 존재하는 이유가 되는 장면` 후보를 찾는다.
- 그 장면이 넓은 시청자에게는 신기함, 애니팬에게는 자기투영, 후속 시청자에게는 `이 사람이 제대로 공부하면 어떻게 될까?`라는 궁금증으로 이어질 수 있는지 본다.
- 콘텐츠 각이 정해지면 그 각을 중심으로 필요한 실제 사건·반응만 선별하고, 필요한 후보에만 실제 일본어 대사 ground truth와 후편집 진단을 붙여 구성한다.
...
즉 현재 기획상 권장 체크포인트는 **0화의 콘텐츠 각과 핵심 장면 확정**이고, 장면 수집기는 **미완료 보존 작업**이다. 사용자가 다른 작업을 선택하면 그 요청을 우선한다.
