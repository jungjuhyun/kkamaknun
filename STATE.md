...
**현재 실행 위치:** **1화 본 촬영 완료 — 분리 녹화본을 현재 primary material로 전환**.  
**다음 실행:** **본 촬영 파일 인벤토리·싱크 확인 → 본 촬영 장면 로깅 → 셀렉트 릴 → 러프 구조표 갱신 → Step 5 품질 판정**.
...
### 2026-09-07 본 촬영 파일 인벤토리 1차 결과

Codex/ffprobe 조사 결과:
- 폴더 `J:\OBS\체인소맨_레제편`에 MP4 8개, 별도 오디오 파일 0개.
- 짧은 테스트 세트 3개는 `camera + window` 페어가 정상이며 48fps, AAC 48kHz stereo, 파일 끝까지 패킷 읽기 성공.
- 본 촬영 시각 `23:54:01`의 `window2026-09-06 23-54-01.mp4`는 약 **1:45:24.625**로 정상 읽힘.
- 같은 시각의 `2026-09-06 23-54-01.mp4`는 약 **5.07GB**이나 `moov atom not found`로 ffprobe 분석 불가.
- 현재 파일 목록은 **사용자가 다른 동시 녹화 파일들을 폴더로 옮기는 중에 조사한 불완전한 스냅샷**이다.
- 따라서 장시간 camera/기타 동시 녹화 파일이 없다고 결론내리지 않는다.
- **파일 이동이 모두 끝난 뒤 전체 폴더 인벤토리를 다시 조사**하고, 그 결과로 정상 세트 구성과 U4의 실제 역할을 판단한다.
- U4의 `moov atom not found`는 현재 관찰된 개별 파일 문제로만 기록하며, 전체 세트 누락/손실로 확대 해석하지 않는다.



- 2026-09-07 사용자 보고: **1화 본 촬영을 새로 완료**했다. 화면 소스와 음성을 분리 녹화하도록 설정한 상태에서 촬영했다. 정확한 파일 구성은 실제 파일 확인 전까지 추정하지 않는다.
- 앞으로 1화 최종 편집/기획 검수는 새 본 촬영을 우선 재료로 사용하고, 기존 약 82분 테스트 촬영은 보조/비교 자료로 둔다.
- **기존 러프 구조표는 본 촬영 전/테스트 재료 기반 가설**이므로 Step 5 최종 판정 입력으로 바로 쓰지 않는다. 본 촬영의 실제 장면을 로깅·셀렉트한 뒤 구조표를 갱신하고 그걸 기준으로 판정한다.
