...
- Phase 4 Actual Work UAT final grader-integrity correction is complete and ready for independent final audit, but the pilot remains `PHASE4_PILOT_NOT_RUN` with `0` Actual Work calls. Initial runnable scenarios remain `P4-A/B/C1/C2/D`; actual bootstrap receipt ref semantics and relation-aware owner/status/historical-precedence grading now prevent positive tokens or later boilerplate from hiding explicit contradictions. The existing raw/hash linkage, marker lifecycle, same-project capability, chronology, independent GitHub evidence, and runnability boundaries remain unchanged.
