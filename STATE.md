...
- 현재 폴더에는 `camera2026-09-06 23-54-01.mp4`라는 장시간 camera 파일이 보이지 않는다.
- 따라서 **장면 로깅 전에 U4의 역할(메인 합성/카메라/오디오 보유 여부)과 비파괴 복구 가능성을 먼저 확인**해야 한다.
- 원본 U4는 절대 수정하지 않고 사본에서만 복구를 시도한다.
