...
**작업 0 — 개발 골격**, **작업 1 — 설정**, **작업 2 — Nadeshiko 실제 연결 확인**, **작업 3 — AI 실제 연결 확인**, **작업 4 — 한국어 표현 찾기**는 완료되어 `main`에 반영됐다.
...
다만 `悪い → 気持ち悪い`, `ほんとそれ → 분리된 ほんと/それ`, `ん？なんて？ → 인용의 なんて`, `今、何してるんですか？ → 비슷한 다른 문장` 같은 **부분 일치 거짓 양성**이 실제로 확인됐다. 따라서 44/50은 유효 표현률이 아니라 일반 검색 회수 수치이며, 현재 병목은 corpus 부족보다 정확 동일표현 검증이다.
현재 바로 다음은 **작업 5 — 정확 동일표현 검사**다.
...
이 값은 확정 계약 시간이 아니라 불확실성 관리용이다. 작업 0~3 기술 확인은 완료됐으며 남은 개발량은 다음 계획 검토에서 다시 산정한다.

## 지금 딱 한 단계

**0화 구성은 계속 유지하면서, 장면 수집기의 Codex 작업 5 — 정확 동일표현 검사를 진행한다.**
...
8. 작업 5 — 표면형 정규화·정확 포함 검사·한자/가나 허용 표기 분리: **다음 작업**.
9. 실제 검색 품질과 검수 병목이 확인된 뒤에만 화면과 추가 자동화를 붙인다.

즉 현재 우선순위는 새로운 거대 자체 시스템을 만드는 것이 아니라 **0화 구성 + 일반 검색의 거짓 양성을 제거하는 최소 정확 동일표현 검증**이다.
