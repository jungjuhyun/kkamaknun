...
**작업 0 — 개발 골격**, **작업 1 — 설정**, **작업 2 — Nadeshiko 실제 연결 확인**, **작업 3 — AI 실제 연결 확인**, **작업 4 — 한국어 표현 찾기**, **작업 5 — 정확 동일표현 검사**는 완료되어 `main`에 반영됐다.
...
다만 `ほんとそれ`, `ん？なんて？`, `今、何してるんですか？`는 상위 20건에서 정확 표면형이 0개였다. 현재 남은 검색 병목은 precision이 아니라 **정확 동일표현이 뒤 페이지에 있을 수 있는데 상위 N건만 보고 없다고 판정할 수 있는 recall 위험**이다.
따라서 작업 6으로 바로 넘어가기 전에 Nadeshiko 공식 `iter_search`/cursor pagination을 이용해 **정확 surface를 필요한 개수만 확보할 때까지 추가 페이지를 보는 최소 recall 검증**을 한 번 수행한다. 별도 새 검색 시스템이나 새 작업 번호를 만들지 않는다.
...
이 값은 확정 계약 시간이 아니라 불확실성 관리용이다. 작업 0~5의 핵심 검색·연결 기술 검증은 완료됐으며, 검색 recall 검증 뒤 기존 계획의 작업 6으로 넘어간다.

## 지금 딱 한 단계

**0화 구성은 계속 유지하면서, 장면 수집기의 작업 6 진입 전 검색 recall 검증을 진행한다.**
...
9. Nadeshiko pagination을 이용한 정확 surface recall 검증: **다음 한 단계**.
10. recall 검증이 충분하면 작업 6 — SQLite·캐시·자료구조 버전으로 이동한다.
11. 실제 검색 품질과 검수 병목이 확인된 뒤에만 화면과 추가 자동화를 붙인다.

즉 현재 우선순위는 새로운 검색 시스템을 만드는 것이 아니라 **0화 구성 + 기존 Nadeshiko pagination으로 정확 동일표현 회수 범위를 최소 검증한 뒤 원래 작업 6으로 복귀하는 것**이다.
