...
- 다음 exact action은 quota가 사용 가능해진 뒤 같은 evaluation snapshot과 lane에서 checkpoint가 지정한 remaining 51 valid trials만 이어서 실행하는 것이다. 완료된 valid trial을 재실행하거나 다른 snapshot/lane과 혼합하지 않으며 Phase 4는 계속 시작하지 않는다.
