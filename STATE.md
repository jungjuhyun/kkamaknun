...
- corrected common surrogate sequencing의 normal continuation은 old prompt archive를 strict하게 거부한다. 다만 `--resume-surrogate-impact`는 authoritative impact verdict와 recovered archive SHA·snapshot·lane·manifest·prompt provenance·영향 범위를 fail-closed로 검증한 뒤 영향을 받지 않은 valid 64개만 release seed로 승계하도록 보정됐다. 따라서 다음 exact action은 동일 snapshot/lane에서 그 seed를 사용해 영향을 받은 bootstrap 30 valid trial과 기존 shortfall 18을 합친 48 valid trial을 새 release evidence로 측정할지 별도 승인받는 것이다. 현재는 raw archive·historical verdict를 보존하고 Phase 4를 시작하지 않는다.
