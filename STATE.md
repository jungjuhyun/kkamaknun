...
현재 기획상 이어가기 좋은 체크포인트는 **0화 ABC 기획 재점검**이다.

`FIRST_VIDEO.md`의 현재 체크포인트를 따른다.
- A/B는 현재 문구로 확정되어 있다.
- C는 미확정이다.
- 현재 기획 흐름에서 다음으로 이어가기 좋은 단계는 윤성원 PD 강의의 의미대로 **시청 후 남을 감상 C를 확정하는 것**이다.
- 이 순서는 **사용자가 다음 작업을 잊지 않도록 제안하는 체크포인트이지 강제 순서가 아니다.** 사용자가 다른 작업을 먼저 하고 싶다면 그 요청을 우선한다.
- AI는 현재 체크포인트를 필요할 때 알려주되, 사용자의 현재 요청을 억지로 C 작업에 연결하거나 다른 작업을 막지 않는다.
- 다른 작업을 먼저 진행해도 이 체크포인트가 폐기되는 것은 아니며, 필요할 때 다시 돌아오면 된다.
- C가 확정된 뒤 콘텐츠 각과 실제 기획 구체화로 이어가는 것이 현재 권장 흐름이지만, 실제 작업 순서는 사용자가 정한다.
...
즉 현재 기획상 권장 체크포인트는 **0화의 감상 C 확정**이고, 장면 수집기는 **미완료 보존 작업**이다. 사용자가 다른 작업을 선택하면 그 요청을 우선한다.
